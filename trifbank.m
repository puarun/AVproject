...
%           Kamil Wojcicki, June 2011
    ...
    for m = 1:M
        ...
        vertexB=(c(m)+c(m+1))/2;
        vertexC=c(m+1);
        k = f>=vertexA&f<=vertexB;          % up-slope
        %H(m,k) = 2*(f(k)-c(m)) / ((c(m+2)-c(m))*(c(m+1)-c(m)));
        H(m,k) = (f(k)-vertexA)/(vertexB-vertexA);%alternative definition
        k = f>=vertexB&f<=vertexC;        % down-slope
        H(m,k) = (vertexC-f(k))/(vertexC-vertexB);% alternative definition
